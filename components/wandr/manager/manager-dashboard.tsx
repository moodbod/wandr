...
import { styles } from '@/components/wandr/manager/manager-dashboard.styles';
...
function BookingRow({
  ...
  const partyLabel = booking.partySize
    ? `${booking.partySize} guest${booking.partySize === 1 ? '' : 's'}`
    : null;
  const requestNote = booking.travelerNote ?? booking.stayBookingDetails?.specialRequest;
    ...
    <View style={[styles.tableRow, { borderColor: colors.borderSoft }]}>
      ...
      <View style={styles.bookingColumn}>
        ...
        <ThemedText numberOfLines={2} style={styles.rowMeta}>
          {[dateLabel, partyLabel, booking.detailLabel ?? booking.subtitle, moneyLabel].filter(Boolean).join(' - ')}
        </ThemedText>
        {requestNote ? (
          <ThemedText numberOfLines={1} style={styles.specialRequest}>{requestNote}</ThemedText>
        ) : null}
      </View>
...
function getManagedBookingDateLabel(booking: ManagedBooking) {
  ...
  if (booking.scheduledFor) {
    return formatter.format(new Date(booking.scheduledFor));
  }
